...
export function PublishAdForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const eventType = String(fd.get("eventType") ?? "club");
    const agePolicy = String(fd.get("agePolicy") ?? "21");
    const details = String(fd.get("details") ?? "").trim();
    const phone = String(fd.get("phone") ?? "").trim();
    const social = String(fd.get("social") ?? "").trim();
    const modNotes = String(fd.get("modNotes") ?? "").trim();

    if (title.length < 4) { setError("Event title is too short."); return; }
    if (venue.length < 2) { setError("Venue name is required."); return; }
    if (address.length < 6) { setError("Add a full address or detailed location patrons can find."); return; }
    if (city.length < 2) { setError("City or region is required."); return; }
    if (!startLocal) { setError("Start date and time are required."); return; }
    if (blurb.length < 24) { setError("Short description should be at least ~24 characters so the board has context."); return; }
    if (!validEmail(email)) { setError("Enter a valid contact email."); return; }
    if (organizer.length < 2) { setError("Organizer or promoter name is required."); return; }
    if (!validUrl(ticketUrl)) { setError("Ticket URL must be http(s), or leave it blank."); return; }
    if (!validUrl(imageUrl)) { setError("Image URL must be http(s), or leave it blank."); return; }
    if (endLocal && endLocal <= startLocal) { setError("End time must be after start time."); return; }
  ...
  if (done) {
    return (
      <div className="rounded-xl border border-[var(--cyan-soft)] bg-[rgba(56,189,248,0.06)] px-4 py-5 text-sm leading-relaxed text-[var(--text)]">
        <p className="font-semibold text-[var(--ok)]">Submission received ✓</p>
        <p className="mt-2 text-[var(--muted)]">
          Your listing is now in the moderation queue. You&apos;ll hear back by email once it&apos;s reviewed — usually within 24 hours.
        </p>
    ...
    <form className="flex flex-col gap-6" onSubmit={onSubmit} noValidate>
      ...
      <div className="grid gap-6 sm:grid-cols-2">
        <div className="flex flex-col gap-2 sm:col-span-2">
          <label htmlFor={idTitle} className={label}>Event title</label>
          <input id={idTitle} name="title" type="text" required placeholder="e.g. Axis Fridays · rooftop sunset" className={field} />
        </div>

        <div className="flex flex-col gap-2">
          <label htmlFor={idVenue} className={label}>Venue name</label>
          <input id={idVenue} name="venue" type="text" required placeholder="Licensed venue or space" className={field} />
        </div>

        <div className="flex flex-col gap-2">
          <label htmlFor={idCity} className={label}>City / area</label>
          <input id={idCity} name="city" type="text" required placeholder="Singapore · Clarke Quay" className={field} />
        </div>

        <div className="flex flex-col gap-2 sm:col-span-2">
          <label htmlFor={idAddress} className={label}>Address or find-me details</label>
          <textarea
            id={idAddress}
            name="address"
            required
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor={idStart} className={label}>Starts (local time)</label>
        ...
        <div className="flex flex-col gap-2 sm:col-span-2">
          <label htmlFor={idType} className={label}>Event type</label>
        ...
        <div className="flex flex-col gap-2 sm:col-span-2">
          <label htmlFor={idBlurb} className={label}>Short board description</label>
          <textarea
            id={idBlurb}
            name="blurb"
            required
        ...
        <div className="flex flex-col gap-2 sm:col-span-2 rounded-xl border border-[var(--border)] bg-[rgba(5,8,14,0.45)] p-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="flex flex-col gap-2">
              <label htmlFor={idOrg} className="text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Promoter / org</label>
              <input id={idOrg} name="organizer" type="text" required className={field} />
            </div>
            <div className="flex flex-col gap-2">
              <label htmlFor={idEmail} className="text-xs font-semibold uppercase tracking-wide text-[var(--muted)]">Email</label>
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor={idAge} className={label}>Age policy at door</label>
      ...
      <fieldset className="flex flex-col gap-3 rounded-xl border border-[var(--border)] bg-[rgba(5,8,14,0.35)] p-4">
        ...
        <label className="flex cursor-pointer items-start gap-3 text-sm text-[var(--muted)]">
          <input id={certRights} name="certRights" type="checkbox" className="mt-1 size-4 shrink-0 rounded border-[var(--border)] accent-[var(--cyan)]" />
          <span>I have rights to the artwork copy and do not infringe third-party trademarks or talent contracts.</span>
        </label>
        <label className="flex cursor-pointer items-start gap-3 text-sm text-[var(--muted)]">
          <input id={certLaw} name="certLaw" type="checkbox" className="mt-1 size-4 shrink-0 rounded border-[var(--border)] accent-[var(--cyan)]" />
          <span>This listing complies with applicable laws (including liquor, safety, and advertising rules).</span>
        </label>
        <label className="flex cursor-pointer items-start gap-3 text-sm text-[var(--muted)]">
          <input id={certModeration} name="certModeration" type="checkbox" className="mt-1 size-4 shrink-0 rounded border-[var(--border)] accent-[var(--cyan)]" />
          <span>
            I understand Party Axis moderates listings and may request edits, delay go-live, or reject submissions — per the{" "}
            <a href="/terms" className="font-semibold text-[var(--cyan)] underline-offset-2 hover:underline">Terms</a>.
          </span>
        </label>
      </fieldset>
      ...
      <button
        type="submit"
        disabled={submitting}
        className="pa-btn-primary w-full rounded-full py-3.5 text-base font-extrabold text-[var(--surface)] no-underline disabled:cursor-not-allowed disabled:opacity-60 sm:w-auto sm:px-12"
      >
      ...
      <p className="text-center text-sm text-[var(--muted)] sm:text-left">
        ...
        — publisher tools connect here when you&apos;re logged in.
      </p>
    </form>
  );
}
